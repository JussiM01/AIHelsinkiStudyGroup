...
% The network can be made without the one hot encoding. In this case the output could
% represent a probabilty estimate for object detection even at a pixel level, which
% could be used for identifing
...
% The summation is over all values of $l$, $m$ and $n$ for which the tensor
...
% As we saw earlier, one drawback of using convolutional layers is that they shrink
% size of the data grid, which opposes limit to the depth of the network. This can
% be compensated by adding artifiacially zeros to the boundary of the input layer.
% This way we can prevent the sizes of the layers shrinking and obtain deeper networks.
% The method is known as zero padding.
% \[
...
% just enough zeros that the layer sizes stays the same after the concolution.
% Namely, if the convolution kernel has spatial width of $k$, then we add to the
% boundary a layer of width $(k - 1)/2$ of zeros.
% Other more extreme option
